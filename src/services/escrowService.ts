...
/** Mock escrows data for development */
const mockEscrows: Escrow[] = [
  {
    id: BigInt(1),
    recipient: 'test-principal-1',
    amount: BigInt(500_000_000_000), // 5,000 DOM
    released_amount: BigInt(200_000_000_000), // 2,000 DOM
    token_type: 'DOM',
    conditions: 'Website redesign project - Phase 1',
    release_authority: { Controller: null },
    status: 'Active',
    created_at: BigInt(Date.now() * 1_000_000 - 30 * 24 * 60 * 60 * 1_000_000_000), // 30 days ago
    expiry: BigInt((Date.now() + 60 * 24 * 60 * 60 * 1000) * 1_000_000), // 60 days from now
    milestones: [
      {
        name: 'Design mockups',
        description: 'Create initial design mockups and wireframes',
        amount: BigInt(100_000_000_000), // 1,000 DOM
        deadline: BigInt((Date.now() - 10 * 24 * 60 * 60 * 1000) * 1_000_000),
        status: 'Released',
        released_at: BigInt((Date.now() - 5 * 24 * 60 * 60 * 1000) * 1_000_000),
      },
      {
        name: 'Frontend implementation',
        description: 'Implement responsive frontend based on approved designs',
        amount: BigInt(200_000_000_000), // 2,000 DOM
        deadline: BigInt((Date.now() + 15 * 24 * 60 * 60 * 1000) * 1_000_000),
        status: 'Approved',
        approved_at: BigInt(Date.now() * 1_000_000),
      },
      {
  ...
  },
  {
    id: BigInt(2),
    recipient: 'test-principal-1',
    amount: BigInt(100_000_000), // 1 ICP
    released_amount: BigInt(100_000_000), // 1 ICP (fully released)
    token_type: 'ICP',
    conditions: 'Community event sponsorship',
    release_authority: { Governance: null },
    status: 'Released',
    created_at: BigInt((Date.now() - 90 * 24 * 60 * 60 * 1000) * 1_000_000), // 90 days ago
    expiry: BigInt((Date.now() - 30 * 24 * 60 * 60 * 1000) * 1_000_000), // Expired 30 days ago
    milestones: [], // Simple escrow without milestones
  },
  {
    id: BigInt(3),
    recipient: 'test-principal-1',
    amount: BigInt(250_000_000_000), // 2,500 DOM
    released_amount: BigInt(0),
    token_type: 'DOM',
    conditions: 'Documentation writing bounty',
    release_authority: { Controller: null },
    status: 'Active',
    created_at: BigInt((Date.now() - 7 * 24 * 60 * 60 * 1000) * 1_000_000), // 7 days ago
    expiry: BigInt((Date.now() + 5 * 24 * 60 * 60 * 1000) * 1_000_000), // Expiring in 5 days
    milestones: [
      {
        name: 'User guide',
        description: 'Complete user documentation for core features',
        amount: BigInt(125_000_000_000), // 1,250 DOM
        deadline: BigInt((Date.now() + 3 * 24 * 60 * 60 * 1000) * 1_000_000),
        status: 'Disputed',
        dispute_reason: 'Scope disagreement - additional sections requested',
      },
      {
        name: 'API reference',
        description: 'Technical API documentation',
        amount: BigInt(125_000_000_000), // 1,250 DOM
        deadline: BigInt((Date.now() + 5 * 24 * 60 * 60 * 1000) * 1_000_000),
        status: 'Pending',
      },
    ],
  },
  {
    id: BigInt(4),
    recipient: 'test-principal-1',
    amount: BigInt(50_000_000_000), // 500 DOM
    released_amount: BigInt(0),
    token_type: 'DOM',
    conditions: 'Marketing campaign - Q1 2024',
    release_authority: { Controller: null },
    status: 'Cancelled',
    created_at: BigInt((Date.now() - 60 * 24 * 60 * 60 * 1000) * 1_000_000), // 60 days ago
    expiry: BigInt((Date.now() - 15 * 24 * 60 * 60 * 1000) * 1_000_000), // Was 15 days ago
    milestones: [],
  },
];
